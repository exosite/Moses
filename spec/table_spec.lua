...
context('Table functions specs', function()
  ...
  context('select', function()
    ...
    test('collects all values passing a truth test with an iterator', function()
      assert_true(_.isEqual(_.filter({1,2,3,4,5,6,7}, function(value,key)
          return (value%2==0)
        end),{2,4,6}))
  ...
  context('all', function()
    ...
    test('returns whether all elements matches a truth test', function()
      assert_true(_.every({2,4,6}, function(value,key)
          return (value%2==0)
        end))
        
      assert_false(_.every({false,true,false}, function(value,key)
          return value == false
        end))        
    end) 
